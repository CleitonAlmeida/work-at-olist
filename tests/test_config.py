...
class TestConfig(unittest.TestCase):
    ...
    def test_flask_env(self):
        ...
        self.assertIn(os.environ.get('FLASK_ENV', None),
                      ['development', 'testing', 'production'])

    def test_db_uri_requirement(self):
        """ Tests SQLALCHEMY_DATABASE_URI environ is not set. """
        # 'sqlite:///:memory:' is the default value that SQLAlchemy
        # set to SQLALCHEMY_DATABASE_URI when is None
        self.assertIsNotNone(os.environ.get('SQLALCHEMY_DATABASE_URI', None))
        self.assertNotEqual(os.environ.get('SQLALCHEMY_DATABASE_URI'),
                            'sqlite:///:memory:')
